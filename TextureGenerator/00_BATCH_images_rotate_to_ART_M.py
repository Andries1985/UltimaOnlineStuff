...
ARTM_folderpath = '/ART_M'  # subdirectory of the generated art_m bmps

noise_amount = 1.0
noise_blend_amount = 0.01  # 0.1
brightness_amount = 0.88  # 0.95 darken originally
...
def tex_to_art_m(target_directory):
    ...
    for infile in glob.glob(os.path.join(target_directory, "*.bmp")):
        ...
        image_main_bgA = ImageOps.fit(image_main_bgA, (artm_high_resolution, artm_high_resolution),
                                      method=Image.BICUBIC, bleed=0.0, centering=(0.5, 0.5))
        image_main_bgB = ImageOps.fit(image_main_bgB, (artm_high_resolution, artm_high_resolution),
                                      method=Image.BICUBIC, bleed=0.0, centering=(0.5, 0.5))
        image_main_bgC = ImageOps.fit(image_main_bgC, (artm_high_resolution, artm_high_resolution),
                                      method=Image.BICUBIC, bleed=0.0, centering=(0.5, 0.5))
        image_main_bgD = ImageOps.fit(image_main_bgD, (artm_high_resolution, artm_high_resolution),
                                      method=Image.BICUBIC, bleed=0.0, centering=(0.5, 0.5))
...
if __name__ == '__main__':
    directory = os.path.dirname(
        os.path.abspath(__file__))  # // local directory , this py is designed to be run in the same folder as bmps
